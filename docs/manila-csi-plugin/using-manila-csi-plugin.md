...
`shareAccessTo` | _no_ | Set access rule entity `access_to` when provisioning a new share. If no value is provided, `access_to` will be set to the name of the volume being created (PersistentVolume name in Kubernetes). This may be beneficial for CephFS shares for example, where Ceph kernel clients using the same cephx ID may re-use MDS cache and OSD connections.
