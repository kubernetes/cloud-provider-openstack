...
func (plugin *cinderPlugin) ConstructVolumeSpec(volumeName, mountPath string) (*volume.Spec, error) {
	...
	hu := kvh.GetHostUtil()
	sourceName, err := hu.GetDeviceNameFromMount(mounter, mountPath, pluginDir)
	if err != nil {
		return nil, err
	}
...
func (b *cinderVolumeMounter) SetUpAt(dir string, mounterArgs volume.MounterArgs) error {
	...
	klog.V(4).Infof("Attempting to mount cinder volume %s to %s with options %v", b.pdName, dir, options)
