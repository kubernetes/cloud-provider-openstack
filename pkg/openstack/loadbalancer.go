...
const (
	...
	pendingDeleteStatus                 = "PENDING_DELETE"
	errorStatus                         = "ERROR"
...
func (lbaas *LbaasV2) GetLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service) (*corev1.LoadBalancerStatus, bool, error) {
	loadbalancer, err := lbaas.getOctaviaLoadBalancer(ctx, clusterName, service)
	if err != nil {
	...
	if portID != "" {
		...
		if err != nil {
			return nil, false, fmt.Errorf("failed to get floating IP for port %s: %w", portID, err)
		}
...
func (lbaas *LbaasV2) getLoadBalancerByName(ctx context.Context, name string, legacyName string) (*loadbalancers.LoadBalancer, error) {
	...
	if len(allLoadbalancers) == 0 {
		if len(legacyName) > 0 {
			...
			opts = loadbalancers.ListOpts{Name: legacyName}
			allLoadbalancers, err = openstackutil.GetLoadBalancers(ctx, lbaas.lb, opts)
			if err != nil {
				return nil, err
			}
	...
	for _, lb := range allLoadbalancers {
		...
		if lb.ProvisioningStatus != deletedStatus && lb.ProvisioningStatus != pendingDeleteStatus {
...
// ensureOctaviaLoadBalancer ensures a load balancer exists and is fully configured for the Service.
// It acquires (creates or reuses) the load balancer, reconciles its listeners, pools and health
// monitors, assigns the external address (floating IP or VIP), and reconciles security groups.
// It supports both owned and shared load balancer scenarios and returns the resulting status.
func (lbaas *LbaasV2) ensureOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node) (lbs *corev1.LoadBalancerStatus, err error) {
	...
	// Populate listeners for an existing LB, or a new one on a provider that creates it unpopulated.
	if !createNewLB || lbaas.opts.ProviderRequiresSerialAPICalls {
...
func (lbaas *LbaasV2) getOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service) (lb *loadbalancers.LoadBalancer, err error) {
	...
	if lbID != "" {
		...
		lb, err = lbaas.getLoadBalancerByName(ctx, name, legacyName)
	}
	if err != nil {
...
// createOctaviaLoadBalancer creates a new Octavia load balancer for the Service and waits until it is
// ACTIVE. It assembles the create options (VIP addressing, tags, flavor, availability zone), and unless
// the provider requires serial API calls, populates the listeners, pools and members in the same request.
// If the load balancer lands in ERROR state it is deleted so the creation can be retried later.
func (lbaas *LbaasV2) createOctaviaLoadBalancer(ctx context.Context, name, clusterName string, service *corev1.Service, nodes []*corev1.Node, svcConf *serviceConfig) (*loadbalancers.LoadBalancer, error) {
	...
	if mc.ObserveRequest(err) != nil {
		var printObj any = createOpts
		if opts, err := json.Marshal(createOpts); err == nil {
			printObj = string(opts)
		}
		return nil, fmt.Errorf("failed to create loadbalancer %v: %w", printObj, err)
	}
	...
	if loadbalancer, err = openstackutil.WaitActiveAndGetLoadBalancer(ctx, lbaas.lb, loadbalancer.ID); err != nil {
		if loadbalancer != nil && loadbalancer.ProvisioningStatus == errorStatus {
			...
			if err = lbaas.deleteLoadBalancer(ctx, loadbalancer, service, svcConf, true); err != nil {
				return nil, fmt.Errorf("loadbalancer %s is in ERROR state and there was an error when removing it: %w", loadbalancer.ID, err)
			}
...
// updateOctaviaLoadBalancer reconciles the pool members of an existing load balancer against the current
// set of nodes. It is called on Node changes, so it only updates members and health monitors per listener
// and reconciles security groups.
func (lbaas *LbaasV2) updateOctaviaLoadBalancer(ctx context.Context, clusterName string, service *corev1.Service, nodes []*corev1.Node) error {
	...
	if svcConf.lbID != "" {
		...
		if err != nil {
			return fmt.Errorf("failed to get load balancer %s: %w", svcConf.lbID, err)
		}
	} else {
	...
	for portIndex, port := range service.Spec.Ports {
		...
		key := listenerKey{proto, int(port.Port)}
		listener, ok := lbListeners[key]
		if !ok {
	...
	if lbaas.opts.ManageSecurityGroups {
		...
		if err != nil {
			return fmt.Errorf("failed to update Security Group for loadbalancer service %s: %w", serviceName, err)
		}
	}
...
// ensureLoadBalancerDeleted removes the resources associated with the Service. If the load balancer is
// owned by this Service and not shared, it deletes the floating IP (unless kept) and the load balancer
// itself; if the load balancer is shared, it only removes the Service's tag. The security group is
// always deleted so it is not orphaned. It is a no-op when the load balancer no longer exists.
func (lbaas *LbaasV2) ensureLoadBalancerDeleted(ctx context.Context, clusterName string, service *corev1.Service) error {
	lbName := lbaas.GetLoadBalancerName(ctx, clusterName, service)
	legacyName := lbaas.getLoadBalancerLegacyName(service)
...
// ==============================================================================
// Listener, Pool, Monitor and Member Management
// ==============================================================================
...
func (lbaas *LbaasV2) filterServiceListeners(listenerList []listeners.Listener, service *corev1.Service, svcConf *serviceConfig) []listeners.Listener {
	...
	for i, l := range listenerList {
		key := listenerKey{listeners.Protocol(l.Protocol), l.ProtocolPort}
	...
	for _, port := range service.Spec.Ports {
		proto := getListenerProtocol(port.Protocol, svcConf)
		listener, isPresent := curListenerMapping[listenerKey{proto, int(port.Port)}]
...
func (lbaas *LbaasV2) checkListenerPorts(service *corev1.Service, curListenerMapping map[listenerKey]*listeners.Listener, isLBOwner bool, lbName string) error {
	for _, svcPort := range service.Spec.Ports {
		key := listenerKey{listeners.Protocol(svcPort.Protocol), int(svcPort.Port)}

		listener, isPresent := curListenerMapping[key]
		if isPresent && !isListenerOwnedByService(*listener, isLBOwner, lbName) {
...
// popListener returns the listeners with the one matching id removed.
func popListener(existingListeners []listeners.Listener, id string) []listeners.Listener {
	var newListeners []listeners.Listener
...
// getListenerProtocol returns the listener protocol to use for a port given the Service configuration.
...
func (lbaas *LbaasV2) ensureOctaviaPool(ctx context.Context, lbID string, name string, listener *listeners.Listener, service *corev1.Service, port corev1.ServicePort, nodes []*corev1.Node, svcConf *serviceConfig) (*v2pools.Pool, error) {
	pool, err := openstackutil.GetPoolByListener(ctx, lbaas.lb, lbID, listener.ID)
	if err != nil && !errors.Is(err, cpoerrors.ErrNotFound) {
		return nil, fmt.Errorf("failed to get pool for listener %s: %w", listener.ID, err)
	}
	...
	if pool != nil && pool.LBMethod != poolLbMethod {
		...
		if err != nil {
			...
			klog.ErrorS(err, "Failed to update LB method for LoadBalancer", "poolID", pool.ID, "listenerID", listener.ID, "lbID", lbID)
			lbaas.eventRecorder.Event(service, corev1.EventTypeWarning, eventLBLbMethodUnknown, fmt.Sprintf("Error updating LB method for LoadBalancer: %v", err))
		} else {
...
// buildPoolCreateOpt builds the pool create options for a listener from the Service configuration.
...
// ensureOctaviaHealthMonitor reconciles the health monitor of a pool to match the Service config:
// it creates one when enabled and missing, deletes it when disabled, or recreates/updates it when the
// existing monitor's type or parameters differ from the desired configuration.
...
// createOctaviaHealthMonitor creates a health monitor for the given pool.
...
// canUseHTTPMonitor reports whether an HTTP health monitor can be used for the given port. It returns
// false on the OVN provider (which has no HTTP monitor support), and for UDP ports it depends on whether
// Octavia supports HTTP monitors on UDP pools.
...
func (lbaas *LbaasV2) buildBatchUpdateMemberOpts(ctx context.Context, port corev1.ServicePort, nodes []*corev1.Node, svcConf *serviceConfig) ([]v2pools.BatchUpdateMemberOpts, sets.Set[string], error) {
	...
	for _, node := range nodes {
		...
		if err != nil {
			if errors.Is(err, cpoerrors.ErrNoAddressFound) {
				// Node failure, do not create member
				klog.Warningf("Failed to get the address of node %s for creating member: %v", node.Name, err)
				continue
			} else {
				return nil, nil, fmt.Errorf("failed to get address of node %s: %w", node.Name, err)
			}
		}
...
// buildCreateMemberOpts builds the pool member create options for the Service port and nodes.
...
// ensureFloatingIP returns the address to advertise in Status.LoadBalancer for the Service. It reuses a
// floating IP already attached to the LB's VIP port, otherwise adopts the one named in Spec.LoadBalancerIP,
// otherwise creates a new one. Internal services get the VIP address and any provider-created FIP is released.
...
// deleteFIPIfCreatedByProvider deletes the floating IP only if it was created by this controller
// (detected via its description), returning whether it was deleted.
func (lbaas *LbaasV2) deleteFIPIfCreatedByProvider(ctx context.Context, fip *floatingips.FloatingIP, portID string, service *corev1.Service) (bool, error) {
	...
	if mc.ObserveRequest(err) != nil {
		return false, fmt.Errorf("failed to delete floating IP %s for loadbalancer VIP port %s: %w", fip.FloatingIP, portID, err)
	}
	klog.InfoS("Deleted floating IP for service", "floatingIP", fip.FloatingIP, "service", klog.KObj(service))
	return true, nil
}

// createFloatingIP creates a floating IP from the given options, logging msg as the reason.
func (lbaas *LbaasV2) createFloatingIP(ctx context.Context, msg string, floatIPOpts floatingips.CreateOpts) (*floatingips.FloatingIP, error) {
	...
	err = PreserveGopherError(err)
	if mc.ObserveRequest(err) != nil {
		return floatIP, fmt.Errorf("failed to create LB floatingip: %w", err)
	}
	return floatIP, err
}

// updateFloatingIP attaches the floating IP to the given port, or detaches it when portID is nil.
func (lbaas *LbaasV2) updateFloatingIP(ctx context.Context, floatingip *floatingips.FloatingIP, portID *string) (*floatingips.FloatingIP, error) {
	...
	if mc.ObserveRequest(err) != nil {
		return nil, fmt.Errorf("failed to update LB floatingip %+v: %w", floatUpdateOpts, err)
	}
...
// checkService validates the Service for load balancer creation and populates svcConf. It rejects
// Services with no nodes or ports, determines the preferred IP family and internal/external mode
// (enforcing internal when the cloud config requires it), and resolves the TLS, network/subnet and
// floating IP configuration before filling in the remaining options via makeSvcConf.
...
// checkServiceUpdate validates the Service for the update path and populates svcConf. It requires at
// least one port, determines the preferred IP family, resolves the member subnet used to register
// nodes, and fills in the remaining options via makeSvcConf.
...
func (lbaas *LbaasV2) resolveMemberSubnetID(ctx context.Context, service *corev1.Service, nodes []*corev1.Node, svcConf *serviceConfig) (string, error) {
	...
	subnetID, err = lbaas.getSubnetIDForLB(ctx, *nodes[0], svcConf.preferredIPFamily)
	if err != nil {
...
// checkServiceDelete populates the minimal svcConf needed for deletion: the load balancer ID, whether
// Octavia tags are supported, and the protocol-affecting flags (keep-client-IP, proxy protocol, TLS ref).
...
// makeSvcConf fills in the remaining svcConf fields from Service annotations and Octavia feature support:
// connection limit, LB method, node selectors, client-IP/proxy settings, timeouts, source ranges, flavor,
// availability zone, TLS, and health monitor parameters. Feature-gated options are skipped (with a warning
// where appropriate) when the Octavia provider does not support them.
func (lbaas *LbaasV2) makeSvcConf(ctx context.Context, serviceName string, service *corev1.Service, svcConf *serviceConfig) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get source ranges for loadbalancer service %s: %w", serviceName, err)
	}
	if openstackutil.IsOctaviaFeatureSupported(ctx, lbaas.lb, openstackutil.OctaviaFeatureVIPACL, lbaas.opts.LBProvider) {
...
func (lbaas *LbaasV2) getSubnetIDForLB(ctx context.Context, node corev1.Node, preferredIPFamily corev1.IPFamily) (string, error) {
	...
	ports, err := getAttachedPorts(ctx, lbaas.network, instanceID)
	if err != nil {
		return "", err
	}
...
// listSubnetsForNetwork lists the subnets of a network, applying any optional list-option tweaks.
func (lbaas *LbaasV2) listSubnetsForNetwork(ctx context.Context, networkID string, tweak ...TweakSubNetListOpsFunction) ([]subnets.Subnet, error) {
	...
	if mc.ObserveRequest(err) != nil {
		return nil, fmt.Errorf("failed to list subnets of network %s: %w", networkID, err)
	}
	subs, err := subnets.ExtractSubnets(allPages)
	if err != nil {
		return nil, fmt.Errorf("failed to extract subnets from pages: %w", err)
	}
...
// nodeAddressForLB returns the node address to register as an LB member, preferring InternalIP over
// ExternalIP as the best guess for an address on the LB's subnet. When preferredIPFamily is set, only an
// address of that family is returned; otherwise cpoerrors.ErrNoAddressFound.
...
// ==============================================================================
// Annotation and Generic Helpers
// ==============================================================================
...
// getKeyValueFromServiceAnnotation parses a comma-separated key=value annotation into a map, using defaultSetting when unset.
...
// updateServiceAnnotation sets an annotation on the Service, initializing the map if needed.
...
// PreserveGopherError unwraps a gophercloud reauthentication error and, for an unexpected-response-code
// error with a known status, appends the response body to the message. Other errors are returned unchanged.
func PreserveGopherError(rawError error) error {
	if rawError == nil {
		return nil
	}
	if v, ok := errors.AsType[gophercloud.ErrErrorAfterReauthentication](rawError); ok {
		rawError = v.ErrOriginal
	}

	var e gophercloud.ErrUnexpectedResponseCode
	ok := errors.As(rawError, &e)
	if !ok {
		return rawError
	}
	...
	case http.StatusUnauthorized,
		...
		if e.Body != nil {
			return fmt.Errorf("%s: %s", rawError, e.Body)
		}
	}
