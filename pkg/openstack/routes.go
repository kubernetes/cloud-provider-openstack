...
func NewRoutes(os *OpenStack, network *gophercloud.ServiceClient, atomicRoutes bool, allowedAddressPairs bool, autoConfigSecurityGroup bool) (cloudprovider.Routes, error) {
	...
	return &Routes{
		...
		autoConfigSecurityGroup: autoConfigSecurityGroup,
	}, nil
}

func (r *Routes) getOrCreateNodeSecurityGroup(clusterName string) error {
	r.Lock()
	defer r.Unlock()
	sgName := fmt.Sprintf("k8s-node-sg-for-cluster-%s", clusterName)
	sgId, err := secgroups.IDFromName(r.network, sgName)
	if err != nil {
...
func (r *Routes) ListRoutes(ctx context.Context, clusterName string) ([]*cloudprovider.Route, error) {
	...
	for _, item := range router.Routes {
		...
		if foundNode {
			...
			port, err := getPortByIP(r.network, item.NextHop, r.networkIDs)
			if err != nil {
				return nil, err
			}
...
func updateSecurityGroup(network *gophercloud.ServiceClient, port *PortWithPortSecurity, sgs []string) (func(), error) {
...
func createSecurityGroupRule(network *gophercloud.ServiceClient, rule rules.CreateOpts) (func(), error) {
	mc := metrics.NewMetricContext("security_group_rule", "create")
	newRule, err := rules.Create(network, rule).Extract()
	if mc.ObserveRequest(err) != nil {
		return nil, err
	}
	unwinder := func() {
		...
		err := rules.Delete(network, newRule.ID).ExtractErr()
		if mc.ObserveRequest(err) != nil {
...
func deleteSecurityGroupRule(network *gophercloud.ServiceClient, rule *rules.SecGroupRule) (func(), error) {
	mc := metrics.NewMetricContext("security-group-rule", "delete")
	err := rules.Delete(network, rule.ID).ExtractErr()
	if mc.ObserveRequest(err) != nil {
		return nil, err
	}
	unwinder := func() {
		...
		_, err := rules.Create(network, rules.CreateOpts{SecGroupID: rule.ID, RemoteIPPrefix: rule.RemoteIPPrefix}).Extract()
		if mc.ObserveRequest(err) != nil {
...
func (r *Routes) CreateRoute(ctx context.Context, clusterName string, nameHint string, route *cloudprovider.Route) error {
	...
	if r.autoConfigSecurityGroup {
		if r.nodeSecurityGroupId == "" {
			err = r.getOrCreateNodeSecurityGroup(clusterName)
			if err != nil {
				return err
			}
		}
	...
	if r.autoConfigSecurityGroup {
		...
		if !nodePortBindSecurityGroup {
			...
			unwind, err := updateSecurityGroup(r.network, port, newSgs)
			if err != nil {
				return err
			}
			defer onFailure.call(unwind)
...
func (r *Routes) DeleteRoute(ctx context.Context, clusterName string, route *cloudprovider.Route) error {
	...
	if staleRule != nil {
		unwind, err := deleteSecurityGroupRule(r.network, staleRule)
		if err != nil {
			return err
		}
		defer onFailure.call(unwind)
	...
	if r.autoConfigSecurityGroup {
		...
		if sgIndex != -1 {
			...
			sgs = sgs[:len(sgs)-1]
			unwind, err := updateSecurityGroup(r.network, port, sgs)
			if err != nil {
				return err
			}
			defer onFailure.call(unwind)
